  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 overflow-hidden">
        ...
        ></div>
        <div
          className={`absolute top-40 right-32 w-24 h-24 rounded-full opacity-30 animate-bounce ${
            isDark ? "bg-pink-400" : "bg-indigo-400"
          }`}
          style={{ animationDelay: "1s" }}
        ></div>
        <div
          className={`absolute bottom-32 left-32 w-20 h-20 rounded-full opacity-25 animate-pulse ${
            isDark ? "bg-cyan-400" : "bg-purple-400"
          }`}
          style={{ animationDelay: "2s" }}
        ></div>
        <div
          className={`absolute bottom-20 right-20 w-28 h-28 rounded-full opacity-20 animate-bounce ${
            isDark ? "bg-emerald-400" : "bg-rose-400"
          }`}
      ...
      <div className="relative z-10 flex flex-col items-center justify-center min-h-screen px-6">
        ...
        <div className="absolute inset-0 pointer-events-none">
          <GraduationCap
            className={`absolute top-32 left-16 w-8 h-8 opacity-60 animate-float ${
              isDark ? "text-blue-400" : "text-blue-600"
            }`}
            style={{ animationDelay: "0s" }}
          />
          <Users
            className={`absolute top-48 right-24 w-6 h-6 opacity-60 animate-float ${
              isDark ? "text-purple-400" : "text-purple-600"
            }`}
            style={{ animationDelay: "1s" }}
          />
          <TrendingUp
            className={`absolute bottom-48 left-24 w-7 h-7 opacity-60 animate-float ${
              isDark ? "text-emerald-400" : "text-emerald-600"
            }`}
            style={{ animationDelay: "2s" }}
          />
          <Zap
            className={`absolute bottom-32 right-16 w-6 h-6 opacity-60 animate-float ${
